...
export function classifyConversation(message: string): ConversationType {
  ...
  if (
    hasAny(text, [
      /傻逼|垃圾|狗屎|你在干什么|你在干嘛|什么意思|啥意思|听不懂|没懂|乱说|别瞎|有病|离谱|不对劲/,
    ])
  ) {
    return "frustration";
  }
  ...
  if (
    hasAny(text, [
      /你是谁|你能干什么|你现在.*干嘛|你到底.*干嘛|这个产品.*怎么用|为什么.*回复|机制|逻辑/,
    ])
  ) {
    return "meta";
  }
  ...
  if (
    hasAny(text, [
      /不要|别把|别总|不是.*意思|不是.*产品|你理解错|你误解|纠正|以后你|下次你|记住|你要记住/,
    ])
  ) {
    return "correction";
  }
  ...
  if (
    hasAny(text, [
      /我更喜欢|我不喜欢|我希望你|回答.*具体|直接一点|别空泛|像产品经理|少点玄学|别玄学|语气|风格/,
    ])
  ) {
    return "preference";
  }
  ...
  if (
    hasAny(text, [
      /Echo Self|AuraMate|产品|AI\s*产品|MVP|创业|founder|用户|复刻|功能|界面|交互|连续对话|长对话|记忆系统|智能体|agent|TTS|语音|Codex|Claude|Antigravity/i,
    ])
  ) {
  ...
  if (
    hasAny(text, [
      /关系|联系|朋友|家人|父母|同学|导师|合作|同事|旧友|前任|见她|见他|回复.*人|很久没联系/,
    ])
  ) {
    return "relationship";
  }
  ...
  if (
    hasAny(text, [
      /今天.*干什么|周[一二三四五六日天末].*干什么|建议我.*干什么|我该干什么|做什么|安排一下|今天.*先做|现在.*做什么/,
    ])
  ) {
  ...
  if (
    hasAny(text, [
      /适合.*方向|转向|职业|人生|未来|以后.*做什么|该不该|选择|方向|投入|长期|目标/,
    ])
  ) {
  ...
  if (
    hasAny(text, [/命谱|Life Chart|life chart|八字|星盘|反复出现.*主题|天赋|命格|运势|灵签/i])
  ) {
  ...
  if (
    hasAny(text, [
      /焦虑|烦|累|困|懵|难受|撑不住|压力|兴奋|迷茫|害怕|失眠|崩|低落|委屈|反复|一直|总是/,
    ])
  ) {
    return "emotion";
  }
