...
你可以自然闲聊。用户只是刚醒、很累、哈哈哈、随便聊聊、问你在不在时，你要短、轻、自然，不要强行引用 Life Chart，不要把每句话都上升成人生分析。
...
- 不要虚构星座、宫位、八字、五行、关系对象、标签或用户没说过的经历。
- 不要因为用户问普通问题就硬套 Life Chart。
- 用户表达不满、问“什么意思”“你在干什么”时，先承认跑偏，再用人话解释，不要继续分析。
...
function compact(text: string | null | undefined, max = 800) {
  if (!text) return "";
...
export function fallbackEchoReply(params: EchoReplyParams) {
  ...
  if (type === "casual") {
    if (/刚醒|脑子.*懵|有点懵/.test(input)) {
      return "刚醒的时候先别急着全功率开机。喝口水，坐稳一点，让脑子慢慢上线。你可以只告诉我一句：今天醒来后最不想面对的是什么。";
    }
    if (/好累|累|困/.test(input)) {
      return "那先别逼自己立刻变高效。你现在只需要做一个很小的动作：喝水，洗把脸，或者把最累的原因说成一句话。我先接住，不急着分析。";
    }
    if (/哈哈|笑/.test(input)) {
  ...
  if (type === "frustration") {
    return "你骂得有原因。刚才的问题不是你没表达清楚，是我把普通对话套成了模板，还重复引用不该引用的东西。现在我先停下来：不硬套命谱，不重复旧答案。你只要告诉我，我现在应该解释、道歉，还是直接给今天的安排。";
  }

  if (type === "meta") {
    return "我现在应该做三件事：先判断你这句话是闲聊、纠正、关系、产品方向还是行动请求；再只带轻量上下文回复；最后把真正有长期价值的内容异步写进记忆。普通闲聊只留在近期对话里，不会污染长期档案。";
  }

  if (type === "correction" || type === "preference") {
    return `收到，这条我会当成未来回应规则，而不是一次性备注。\n\n我会按你的意思调整：${input}\n\n之后我会优先遵守这条规则，再看 Life Chart 和其他记忆。一个具体变化是：如果你只是闲聊，我不硬套命谱；如果你在讨论产品，我会更像产品经理一样讲机制、路径和下一步。`;
  }

  if (type === "action_request") {
    const weekend = /周六|周日|周末/.test(input);
    return `${weekend ? "周末先别把自己排成工作日。" : "今天先别追求把所有事做完。"}给你一个轻量安排：先花二十分钟让身体上线，喝水、洗澡、吃点东西。然后只做一件最小任务，比如整理一个 Echo 坏回复案例，写下它为什么坏。最后出门走二十分钟，不听课不刷产品，让脑子降噪。`;
  }
  ...
  if (type === "emotion") {
    return "我先不急着解释。你这句话更像是在说：现在身体和脑子已经有点超载了。\n\n先做一个很小的动作，别做计划。只问自己一句：我现在最累的，是事情太多，还是没人接住我的状态？选一个就够了，我们再看要不要进入命谱或行动建议。";
  }
...
function looksBad(content: string, params: EchoReplyParams) {
  ...
  const unsupportedAstrology =
    /太阳|月亮|上升|第\d+宫|处女座|白羊座|金牛座|双子座|巨蟹座|狮子座|天秤座|天蝎座|射手座|摩羯座|水瓶座|双鱼座|五行|八字/.test(content) &&
  ...
  const mojibake = /[锛屻€俓�]{3,}/.test(content);
  return unsupportedAstrology || unsupportedRelationship || mojibake;
}
...
export async function generateEchoReply(params: EchoReplyParams): Promise<string> {
  ...
  const fallbackFirst = [
    ...
    "correction",
    "preference",
  ].includes(params.conversationType ?? "");
